...
namespace TensorII::Core {
    ...
    {
        return Tensor<DType, Shape<0>{}>(TensorInitializer<DType, Shape<0>{}>(value));
    }
    ...
    constexpr auto toTensor(const DType (&array)[dimension]) // -> Tensor<DType, Shape<1>{dimension}, Allocator>
    {
        ...
        return Tensor<DType, shape_>(TensorInitializer<DType, Shape<1>{dimension}>(array));
    }
    ...
    constexpr auto toTensor(const DType (&array)[d1][d2]) // -> Tensor<DType, Shape<2>{d1, d2}, Allocator>
    {
        ...
        return Tensor<DType, shape_> (TensorInitializer<DType, shape_>(array));
    }
